...
<div class="gsp_indentedContent">
  <p class="gsp_a_ap_to">
    <asp:Label ID="lbl1" runat="server" CssClass="gsp_bold" Text="<%$ Resources:GalleryServerPro, Admin_Settings_Apply_To_Label %>"
      EnableViewState="false" />&nbsp;<asp:Literal ID="Literal2" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_All_Galleries_Label %>" />
  </p>
  ...
  <div class="gsp_addleftpadding5">
    <div class="gsp_single_tab">
      <div class="gsp_single_tab_hdr">
        <span class="gsp_single_tab_hdr_txt">
          <asp:Label ID="lblCurrentGalleryHdr" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Gallery_Settings_CurrentGallery_Hdr %>" />
        </span>
      </div>
    ...
    <div class="gsp_single_tab">
      <div class="gsp_single_tab_hdr">
        <span class="gsp_single_tab_hdr_txt">
          <asp:Label ID="lblManageGalleriesHdr" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Gallery_Settings_Galleries_Hdr %>" />
        </span>
      </div>
      <div class="gsp_single_tab_bdy gsp_dropshadow3">
              ...
              <ItemTemplate>
                <div class="gsp_g_edit_cell">
                  ...
                  <asp:LinkButton ID="lbDeleteGallery" runat="server" CausesValidation="False" CommandName="Delete"
                    ...
                    ToolTip="<%$ Resources:GalleryServerPro, Admin_Gallery_Settings_Delete_Gallery_Link_ToolTip %>"></asp:LinkButton>
                </div>
              </ItemTemplate>
            </asp:TemplateField>
  ...
  <script>
    (function ($) {
      ...
      var configTooltips = function () {
        $('#<%= lblCurrentGalleryHdr.ClientID %>').gspTooltip({
			    title: '<%= Resources.GalleryServerPro.Admin_Gallery_Settings_CurrentGallery_Info_Hdr.JsEncode() %>',
				  content: '<%= Resources.GalleryServerPro.Admin_Gallery_Settings_CurrentGallery_Info_Bdy.JsEncode() %>'
				});

			  $('#<%= lblManageGalleriesHdr.ClientID %>').gspTooltip({
			    title: '<%= Resources.GalleryServerPro.Admin_Gallery_Settings_ManageGalleries_Info_Hdr.JsEncode() %>',
			  content: '<%= Resources.GalleryServerPro.Admin_Gallery_Settings_ManageGalleries_Info_Bdy.JsEncode() %>'
			});
			};
