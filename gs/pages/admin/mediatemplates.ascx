<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="mediatemplates.ascx.cs" Inherits="GalleryServerPro.Web.Pages.Admin.mediatemplates" %>
...
<div class="gsp_indentedContent" runat="server">
  ...
  <div class="gsp_addleftpadding5">
    <p>
      MIME type:&nbsp;
      <asp:DropDownList ID="ddlMimeType" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlMimeType_SelectedIndexChanged" />
    </p>
    <p>
    ...
    (function ($) {
      ...
      var configTooltips = function () {
        $('.gsp_admin_h2_txt', '#<%= cid %>').gspTooltip({
          title: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Overview_Hdr.JsEncode() %>',
          content: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Overview_Bdy.JsEncode() %>'
        });
        
        $('.gsp_mt_mt_h', '#<%= cid %>').gspTooltip({
          title: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_MimeType_Hdr.JsEncode() %>',
          content: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_MimeType_Bdy.JsEncode() %>'
        });
        
        $('.gsp_mt_bid_h', '#<%= cid %>').gspTooltip({
          title: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_BrowserId_Hdr.JsEncode() %>',
          content: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_BrowserId_Bdy.JsEncode() %>'
        });
         
        $('.gsp_mt_html_h', '#<%= cid %>').gspTooltip({
          title: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Tmpl_Hdr.JsEncode() %>',
          content: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Tmpl_Bdy.JsEncode() %>'
        });
       
        $('.gsp_mt_js_h', '#<%= cid %>').gspTooltip({
          title: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Tmpl_Hdr.JsEncode() %>',
          content: '<%= Resources.GalleryServerPro.Admin_MediaTmpl_Tmpl_Bdy.JsEncode() %>'
        });
      };
