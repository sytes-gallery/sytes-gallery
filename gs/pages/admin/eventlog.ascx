<%@ Control Language="C#" AutoEventWireup="True" CodeBehind="eventlog.ascx.cs" Inherits="GalleryServerPro.Web.Pages.Admin.eventlog" %>
...
<div class="gsp_indentedContent">
  <p class="gsp_a_ap_to">
    <asp:Label ID="lbl1" runat="server" CssClass="gsp_bold" Text="<%$ Resources:GalleryServerPro, Admin_Settings_Apply_To_Label %>"
      EnableViewState="false" />&nbsp;<asp:Literal ID="Literal1" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_All_Galleries_User_Is_Admin_For_Label %>" />
  </p>
  ...
  <div class="gsp_addleftpadding5">
    ...
    <asp:Repeater ID="rptr" runat="server" ItemType="GalleryServerPro.Business.Interfaces.IEvent" EnableViewState="false">
      <HeaderTemplate>
      ...
      <ItemTemplate>
        <tr data-id="<%# Item.EventId %>">
          <td><a href="#" class="gsp_el_dtl_btn gsp_el_dtl_clp gsp_hoverLink" title="Show event details"></a>
	...
	<script>
	  (function ($) {
	    $(document).ready(function() {
	      ...
	      $(".gsp_el_dtl_btn", <%= cid %>.p()).click(function(e) {
	        var el = $(this);
	        var elRow = $(this).closest('tr');
	        
	        el.toggleClass('gsp_el_dtl_clp gsp_el_dtl_shw');
	        
	        if (el.hasClass('gsp_el_dtl_shw')) {
	          // Show event detail
	          $('.gsp_el_et', elRow).addClass('gsp_wait_center');
	          $.get(window.Gsp.AppRoot + '/api/events/' + elRow.data('id'), function(html) {
	      ...
	      $(".gsp_el_d_btn", <%= cid %>.p()).click(function(e) {
	        var elRow = $(this).closest('tr');
	        $('.gsp_el_et', elRow).addClass('gsp_wait_center');
	        
	        $.ajax(({
	          type: "DELETE",
	          url: window.Gsp.AppRoot + '/api/events/' + elRow.data('id'),
	          success: function (o) {
	            elRow.next('.gsp_el_dtl').addBack().remove();
	            checkForEmptyLog();
	          },
	          error: function (response) {
	            $.gspShowMsg("Cannot Delete", response.responseText, { msgType: 'error', autoCloseDelay: 0 });
	          }
	        }));
	        return false;
	      });
