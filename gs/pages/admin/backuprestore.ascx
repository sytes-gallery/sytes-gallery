<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="backuprestore.ascx.cs" Inherits="GalleryServerPro.Web.Pages.Admin.backuprestore" %>
...
<div class="gsp_indentedContent">
	<p class="gsp_a_ap_to">
		<asp:Label ID="lbl1" runat="server" CssClass="gsp_bold" Text="<%$ Resources:GalleryServerPro, Admin_Settings_Apply_To_Label %>"
			EnableViewState="false" />&nbsp;<asp:Literal ID="Literal1" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_All_Galleries_Label %>" />
	</p>
			...
			<div id="<%= cid %>_bakRestoreTabContainer" class="gsp_addtoppadding5 gsp_tabContainer">
				<ul>
					<li><a href="#<%= cid %>_bakRestoreTabBackup">
						<asp:Literal ID="l1" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Backup_Tab_Title %>" /></a></li>
					<li><a href="#<%= cid %>_bakRestoreTabRestore">
						<asp:Literal ID="l2" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Title %>" /></a></li>
				</ul>
				<div id="<%= cid %>_bakRestoreTabBackup" class="ui-corner-all">
					<p class="gsp_h3">
						<asp:Literal ID="l1b" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Backup_Tab_Hdr %>" />
					</p>
					<p class="gsp_addtopmargin5">
						<asp:Label ID="lblBackupDtl" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Backup_Tab_Dtl %>" />
					</p>
					<div class="gsp_msgBoxInfo">
						<asp:Literal ID="l16" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Backup_Tab_Note %>" />
					</div>
					<p class="gsp_addtopmargin5">
				...
				<div id="<%= cid %>_bakRestoreTabRestore" class="ui-corner-all">
					...
					<p>
						<asp:Literal ID="l4" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Dtl %>" />
					</p>
					<div class="gsp_msgBoxInfo">
						<asp:Literal ID="l15" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Note %>" />
					</div>
					<p>
						<asp:FileUpload ID="fuRestoreFile" runat="server" size="45" CssClass="gsp_textbox" ToolTip="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Upload_File_Browse_Button_Tooltip %>" />&nbsp;
						<asp:Button ID="btnUpload" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Upload_File_Button_Text %>" ToolTip="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Upload_File_Button_Tooltip %>" OnClick="btnUpload_Click" />
					</p>
							...
							<p style="width: 400px;">
								<span class="gsp_fs gsp_msgwarning"><span class="gsp_bold">
									<asp:Literal ID="l9" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Restore_Warning_Hdr %>" /></span>&nbsp;<asp:Literal
										ID="l10" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Backup_Restore_Restore_Tab_Restore_Warning_Dtl %>" /></span>
							</p>
						</div>
					</asp:Panel>
				</div>
	...
	<script>
		(function ($) {
		  ...
		  var configTooltips = function() {
		    $('#<%= lblBackupDtl.ClientID %>').gspTooltip({
		      title: '<%= Resources.GalleryServerPro.Admin_BackupRestore_Backup_Overview_Hdr.JsEncode() %>',
		      content: '<%= Resources.GalleryServerPro.Admin_BackupRestore_Backup_Overview_Bdy.JsEncode() %>'
		    });
		    
		    $('#<%= chkExportMembership.ClientID %>').gspTooltip({
		      title: '<%= Resources.GalleryServerPro.Admin_BackupRestore_ExportUserAccounts_Hdr.JsEncode() %>',
		      content: '<%= Resources.GalleryServerPro.Admin_BackupRestore_ExportUserAccounts_Bdy.JsEncode() %>'
		    });
		    
		    $('#<%= chkExportGalleryData.ClientID %>').gspTooltip({
		      title: '<%= Resources.GalleryServerPro.Admin_BackupRestore_ExportGalleryData_Hdr.JsEncode() %>',
		      content: '<%= Resources.GalleryServerPro.Admin_BackupRestore_ExportGalleryData_Bdy.JsEncode() %>'
		    });
		  };
