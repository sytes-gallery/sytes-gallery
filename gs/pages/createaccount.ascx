...
<asp:Panel ID="pnlCreateUser" runat="server" CssClass="gsp_createuser gsp_rounded10"
  ...
  <p class="gsp_h1">
    <img src="<%= Utils.SkinPath %>/images/user-l.png" alt="" class="gsp_ca_u_img" /><asp:Literal ID="litHeader" runat="server" Text="<%$ Resources:GalleryServerPro, CreateAccount_HeaderNormal %>" />
  </p>
    ...
    <tr>
      <td colspan="2">
        <p class="gsp_fs gsp_msgwarning">
          <asp:Literal ID="l6" runat="server" Text="<%$ Resources:GalleryServerPro, Site_Field_Required_Text %>" />
        </p>
      </td>
    </tr>
  </table>
    ...
    (function ($) {
      ...
      var bindEventHandlers = function () {
        $('#<%= txtNewUserUserName.ClientID %>').change(function () {
          ...
          $.ajax(({
            type: "GET",
            url: window.Gsp.AppRoot + '/api/users/exists?userName=' + encodeURIComponent(userName),
            success: function (userExists) {
              if (userExists)
                msg = '<span class="gsp_addleftpadding1 gsp_msgwarning"><%= Resources.GalleryServerPro.Site_UserNameIsDuplicate %></span>';
              else
                msg = '<span class="gsp_addleftpadding1 gsp_msgfriendly"><%= Resources.GalleryServerPro.Site_UserNameAvailable %></span>';
